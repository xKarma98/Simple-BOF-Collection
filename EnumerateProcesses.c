...
typedef DWORD(WINAPI* PPN_GetLastError)(void);

void exec() {
	HMODULE sas = LoadLibraryA("kernel32.dll");
	FARPROC yes = GetProcAddress(sas, "GetLastError");
	PPN_GetLastError pGetLastError = (PPN_GetLastError)(GetProcAddress(sas, "GetProcAddress"));

	// Load dll
	HMODULE hat = LoadLibraryA("Wtsapi32.dll");
	if (!hat) {
		BeaconPrintf(CALLBACK_ERROR, "Issue importing the dll: %lu", pGetLastError());
	}
	else {
		BeaconPrintf(CALLBACK_OUTPUT, "Successfully loaded the DLL\n");
	}
	...
	if (!ser) {
		BeaconPrintf(CALLBACK_ERROR, "Failure to get a handle: %lu", pGetLastError());
	}
	else {
	...
	if (!da) {
		BeaconPrintf(CALLBACK_ERROR, "Failed to enumerate sessions: %lu", pGetLastError());
	}
	else {
	...
	for (DWORD i = 0; i < pcount; i++) {
		...
		if (pLookupAccountSidA(NULL, usersid, name, &cchname, domain, &cchrefdomain, &suse)) {
			...
			BeaconPrintf(CALLBACK_ERROR, "Failed to get account sid's %lu", pGetLastError());
		}
	}
